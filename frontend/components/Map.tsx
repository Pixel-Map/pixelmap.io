...
export default function Map(props: any) {
  ...
  useEffect( () => {
    setOwnedTiles(props.tiles.filter( (tile: TileAsset) => {
      return (account && account != "" && account.toLowerCase() === tile.owner.toLowerCase() )
    }));

    setForSaleTiles(props.tiles.filter( (tile: TileAsset) => {
      return (tile.price != "0" || ( tile.openseaPrice && tile.openseaPrice != "0") );
    }));
  }, [props.tiles, account]);
  ...
  return (
    <>
      <div className="max-w-map mx-auto px-3 flex justify-between">
        <MapToggles showOwned={showOwned} setShowOwned={toggleOwnedTiles} showForSale={showForSale} setShowForSale={toggleForSale} />

        {/* <ToggleTheme theme={theme} toggleTheme={toggleTheme} /> */}
      </div>
      
      <div className="overflow-auto p-2" style={{touchAction: "manipulation"}}>
        <div 
        ...
        >
          ...
          <Image className="absolute inset-0 z-10" src={`${publicRuntimeConfig.NEXT_PUBLIC_API_ENDPOINT}/tilemap`}  alt="PixelMap Map" layout="fill" />
        </div>
      </div>
    </>
  );
