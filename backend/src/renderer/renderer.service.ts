	...
	async renderImages() {
		if (!this.currentlyReadingImages) {
			try {
				...
				const previousTiles = [];
				// Reload latest data
				const tiles = await this.tileData.find(
					{},
					{ orderBy: { id: QueryOrder.ASC } },
				);
				for (let i = 0; i < tiles.length; i++) {
					previousTiles[tiles[i].id] = tiles[i].image;
				}
				...
				const tileDataChange = await this.dataHistory.find(
					{},
					{
						populate: ["tile"],
				...
				for (let i = lastEvent.value; i < tileDataChange.length; i++) {
					...
					if (imageData.length === 768) {
						this.logger.verbose(
							`Saving image of tile: ${tileData.tile.id} at block: ${tileData.blockNumber}`,
						);
						await renderImage(
							imageData,
							512,
							512,
							`cache/${tileData.tile.id}/${tileData.blockNumber}.png`,
						);
						await renderFullMap(
							previousTiles,
							`cache/fullmap/${tileData.blockNumber}.png`,
						);
						this.logger.verbose(
				...
				for (let i = 0; i <= 3969; i++) {
					...
					if (imageData.length === 768) {
						const lastRender = await this.cacheManager.get(
							`image-${String(tiles[i].id)}`,
						);
