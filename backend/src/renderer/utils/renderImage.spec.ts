...
it('successfully renders a PNG image from a compressed image string', async () => {
  ...
  await renderImage(decompressTileCode(compressedImage), 512, 512, 'cache/test.png');
...
it('successfully renders a PNG image from tile #793', async () => {
  ...
  await renderImage(decompressTileCode(compressedImage), 48, 48, 'cache/test-793.png');
  expect(decompressTileCode(compressedImage)).toBe(
    // eslint-disable-next-line max-len
...
it('successfully renders a PNG image from tile #293', async () => {
  ...
  await renderImage(decompressTileCode(compressedImage), 16, 16, 'cache/test-293.png');
  expect(decompressTileCode(compressedImage)).toBe(
    // eslint-disable-next-line max-len
