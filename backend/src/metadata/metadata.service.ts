...
export class MetadataService {
	...
	async generateTiledataJSON(tiles: Tile[]) {
		...
		await fs.writeFileSync(
			"cache/tiledata.json",
			JSON.stringify(tiledataJSON, null, 2),
		);
	}
	...
	async updateTileMetadata(tile: Tile) {
		...
		const ogTiles = [
			0, 80, 574, 868, 1317, 1416, 1661, 1822, 1901, 1902, 1903, 1904, 1905,
		...
		const previousMetadata = await this.cacheManager.get(`metadata-${tile.id}`);
		if (previousMetadata === jsonMetaData) {
			// this.logger.verbose("Not re-saving metadata, it's identical!");
		} else {
			await fs.writeFileSync(
				`cache/metadata/${tile.id}.json`,
				JSON.stringify(tileMetaData, null, 2),
			);
			// Write data for /tile endpoint
			await fs.writeFileSync(
				`cache/tile/${tile.id}.json`,
				JSON.stringify(pixelMapTile, null, 2),
			);
			await this.cacheManager.set(`metadata-${tile.id}`, jsonMetaData);
		}
	}
}
