...
export class SyncToS3Service {
  ...
  async regenerateAPIData() {
    ...
    await fs.writeFileSync('cache/allimages.json', JSON.stringify(allImages, null, 2));

    this.logger.verbose('Regenerating timecapsule data');
    const timeCapsule = await this.getFirstTilesWithImages();
    await fs.writeFileSync('cache/timecapsule.json', JSON.stringify(timeCapsule, null, 2));

    this.logger.verbose('Regenerating owners data');
    const owners = await this.getOwners();
    await fs.writeFileSync('cache/owners.json', JSON.stringify(owners, null, 2));
  }
