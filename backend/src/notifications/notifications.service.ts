...
import {
	CurrentState,
	StatesToTrack,
} from "../ingestor/entities/currentState.entity";
...
export class NotificationsService {
	...
	async eventsToDiscordNotifications() {
		if (!this.currentlyReadingEventsIntoDiscord) {
			...
			const dataHistory = await this.dataHistory.find(
				{},
				{
					populate: ["tile"],
			...
			for (let i = lastEvent.value; i < dataHistory.length; i++) {
				...
				const ChannelWantSend =
					await this.client.channels.fetch("880262613664137270");
				let ownerName = undefined;
				try {
					ownerName = await lastValueFrom(
						this.httpService.get(
							`https://api.opensea.io/user/${updatedTile.tile.owner}?format=json`,
						),
					);
				} catch {}
				...
				const message = new EmbedBuilder()
					.setColor("#66ff82")
					.setTitle(
						`Tile #${updatedTile.tile.id} has been updated by ${updatedBy}!`,
					)
					.setAuthor({
						name: "PixelMap",
						iconURL: "https://pixelmap.io/assets/images/logo-color.png",
						url: "https://pixelmap.io",
					})
					.setDescription(getCompliment())
					.setFooter({
						text: `...And they've chosen to link it to: ${sanitizedURL}`,
					})
					.setURL(sanitizedURL)
					.setThumbnail(link);
				if (updatedTile.tile.image.length !== 0) {
					try {
						...
						console.log(
							"Failed to send notification because of bad URL, trying again!",
						);
				...
				await this.currentState.getEntityManager().persistAndFlush(lastEvent);
			}
